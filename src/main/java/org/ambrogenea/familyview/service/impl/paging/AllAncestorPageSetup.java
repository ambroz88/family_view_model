...
public class AllAncestorPageSetup implements Pageable {

    private final Paging paging;
    private final ConfigurationService config;

    public AllAncestorPageSetup(ConfigurationService config) {
        this.config = config;
    ...
    private int calculateXPosition(AncestorPerson personModel) {
        return paging.calculateAllAncestorsX(personModel);
    }
    ...
    private int calculatePageHeight(AncestorPerson personModel) {
        int height = paging.calculateLineageHeight(personModel);
        if (config.isShowCouplesVertical()) {
            height = height - 2 * (config.getAdultImageHeight());
        }
        return height;
    }

    private int calculatePageWidth(AncestorPerson personModel) {
        return paging.calculateAllAncestorsWidth(personModel);
    }
